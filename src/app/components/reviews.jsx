...
const Reviews = ({ productName }) => {
  ...
  const [isReviewVisible, setIsReviewVisible] = useState(false);
   useReviewVisibility = (bool) => {
    setIsReviewVisible(bool)
    ...
    <>
      {isFormVisible && (
        ...
        <form id="reviewForm" style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          ...
          <button type="submit" id="postReview" style={{ border: '0', background: '#111', color: '#fff', padding: '0.68em', borderRadius: '14px', fontWeight: 'bold' }} onClick={()=>{submitReview(event)}}>Submit</button>
        </form>
      </div>
      )}
      {isReviewVisible &&
...
>
  X
    ...
    textDecoration: "underline"}}>Based on {countReviews} reviews</p>
        </div>
      </div>
...
>
  <span>
    {Array(Math.round(averageRating))
      .fill("★") // Create filled stars based on the rating
      .join("")}
    {Array(5 - Math.round(averageRating))
      .fill("☆") // Create unfilled stars for the remaining count
      .join("")}
  </span>
</div>
